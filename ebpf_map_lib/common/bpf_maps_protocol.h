...
#define IOCTL_BPF_MAP_RINGBUF_MAP \
    CTL_CODE(FILE_DEVICE_NETWORK, 0x906, METHOD_BUFFERED, FILE_ANY_ACCESS)

#define IOCTL_BPF_MAP_SET_WAIT_HANDLE \
    CTL_CODE(FILE_DEVICE_NETWORK, 0x907, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
// Ring buffer map operation
typedef struct _bpf_map_ringbuf_map_request {
    uint64_t map_handle;
} bpf_map_ringbuf_map_request_t;

typedef struct _bpf_map_ringbuf_map_reply {
    uint64_t consumer_address;
    uint64_t producer_address;
    uint64_t data_address;
    size_t data_size;
    uint64_t event_handle;
} bpf_map_ringbuf_map_reply_t;

// Set wait handle operation
typedef struct _bpf_map_set_wait_handle_request {
    uint64_t map_handle;
    uint64_t event_handle;
} bpf_map_set_wait_handle_request_t;
