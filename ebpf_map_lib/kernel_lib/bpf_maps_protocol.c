    ...
    _Out_ PULONG bytes_returned)
{
    ...
    *bytes_returned = 0;
    ...
    case IOCTL_BPF_MAP_RINGBUF_MAP: {
        bpf_map_ringbuf_map_request_t* request = (bpf_map_ringbuf_map_request_t*)input_buffer;
        bpf_map_ringbuf_map_reply_t* reply = (bpf_map_ringbuf_map_reply_t*)output_buffer;

        if (input_length < sizeof(bpf_map_ringbuf_map_request_t) ||
            output_length < sizeof(bpf_map_ringbuf_map_reply_t)) {
            return STATUS_BUFFER_TOO_SMALL;
        }
        ...
        // Map ring buffer - call kernel function (receives handle directly)
        status = bpf_map_ringbuf_map_km(
            request->map_handle,
            &reply->consumer_address,
            &reply->producer_address,
            &reply->data_address,
            &reply->data_size,
            &reply->event_handle);

        if (NT_SUCCESS(status)) {
            *bytes_returned = sizeof(bpf_map_ringbuf_map_reply_t);
        }

        break;
    }

    case IOCTL_BPF_MAP_SET_WAIT_HANDLE: {
        bpf_map_set_wait_handle_request_t* request = (bpf_map_set_wait_handle_request_t*)input_buffer;

        if (input_length < sizeof(bpf_map_set_wait_handle_request_t)) {
            return STATUS_BUFFER_TOO_SMALL;
        }
        ...
        // Set wait handle - call kernel function (receives handle directly)
        status = bpf_map_set_wait_handle_km(request->map_handle, request->event_handle);
